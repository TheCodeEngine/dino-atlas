...
function PieceTile({ piece, size }: { piece: number; size: number }) {
...
function DraggablePiece({ piece, size }: { piece: number; size: number }) {
...
function DroppableSlot({ position, piece, size }: { position: number; piece: number | null; size: number }) {
...
export function PuzzleScreen() {
  ...
  return (
    <div className="bg-[#2C1A0E] text-white min-h-screen flex flex-col" style={{ backgroundImage: "none" }}>
      <FullscreenHeader title="Skelett-Puzzle" playerEmoji="🦖" variant="dark" />
        ...
        <div className="mx-4 mb-3 relative">
          ...
          >
            {Array.from({ length: TOTAL }).map((_, pos) => (
              <DroppableSlot key={pos} position={pos} piece={board[pos] ?? null} size={pieceSize} />
            ))}
          </div>
        ...
        {!complete && (
          <div className="px-4 pb-4">
            <div className="flex flex-wrap justify-center gap-1.5">
              {tray.map((piece) => (
                <DraggablePiece key={piece} piece={piece} size={pieceSize} />
              ))}
            </div>
          </div>
        )}
        ...
        <DragOverlay>
          {activePiece !== null && (
            <div className="rounded-lg overflow-hidden border-[3px] border-[#ffc850] shadow-xl" style={{ width: pieceSize, height: pieceSize }}>
              <PieceTile piece={activePiece} size={pieceSize} />
            </div>
          )}
        </DragOverlay>
      </DndContext>
